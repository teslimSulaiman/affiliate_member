...
class Edit extends Action
{
    ...
    protected function _initAction()
    {
        ...
        $resultPage->setActiveMenu('Test_Affiliate::member')
            ->addBreadcrumb(__('Member'), __('Member'))
            ->addBreadcrumb(__('Manage Members'), __('Manage Members'));
        return $resultPage;
    }
    ...
    public function execute()
    {
        ...
        if ($id) {
            $model->load($id);
            if (!$model->getId()) {
                $this->messageManager->addError(__('This member not exists.'));
        ...
        $this->_coreRegistry->register('affiliates_member', $model);
        ...
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? $model->getName() : __('New Member'));
